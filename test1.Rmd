...
str(murders)
```
Answer: c.The state name, the abbreviation of the state name, the state’s region, and the state’s population and total number of murders for 2010.
...
identical(a,b)
```
5. We saw that the region column stores a factor. You can corroborate this by typing: class(murders$region).With one line of code, use the function levels and length to determine the number of regions defined by this dataset.
...
city_temps <- data.frame(name = city, temperature = new_temp)
city_temps
```
...
```{r}
table(murder_rate<mean(murder_rate))
```
...
```{r}
murders$state[which(murders$abb%in%c("MA","ME","MI","MO","MU"))]
```
...
```{r}
population_in_millions <- log(murders$population,10)
total_gun_murders <- murders$total
plot(population_in_millions, total_gun_murders)
```
